...
timestamp_for_last_update = f"{datetime.now().strftime('%X')} on {datetime.now().strftime('%B')} {int(datetime.now().strftime('%d'))}, {int(datetime.now().strftime('%Y'))}"

new_markdown_text = markdown_text.replace("{random_sighting}", markdown_text_for_template)

new_markdown_text = new_markdown_text.replace("{timestamp_for_last_update}", timestamp_for_last_update)
